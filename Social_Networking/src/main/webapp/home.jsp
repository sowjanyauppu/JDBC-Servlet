...
<body>
	...
	<form action="<%=request.getContextPath()%>/Search" method="post">
		Find new friends having age from <input type="text"
					...
					<select name="gender"><option value="">Any</option><option value="M">Male</option><option value="F">Female</option></select> with marital status as 
					<select name="marital"><option value="">Any</option><option value="S">Single</option><option value="M">Married</option></select> people.<br/><br/>Also, i like the 
	...
	<div class="col-lg-12 row">
	 <% 
	 	List<UserDetails> users = (List<UserDetails>)session.getAttribute("userList");
...
<%}else if(user.getGender().trim().startsWith("F")){ %>
...
<%}%>
	 	...
	 	<%}else{%>
	 	<div class="text-center"><script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
<lottie-player src="https://assets6.lottiefiles.com/packages/lf20_kay5m4bw.json"  background="transparent"  speed="1"  style="width: 300px; height: 300px;"  loop  autoplay></lottie-player>
	 	...
	 	<div><h5>Oh! seems no one meet your requirement. Try to adjust your requirement to get more connections...!</h5></div>
	 <%}%>
	</div>
	</div>
