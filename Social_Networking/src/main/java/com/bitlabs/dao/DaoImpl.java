...
public class DaoImpl {
 ...
 query="select * from user where Id in (select f.friend_id from user u inner join friends f on f.user_id = u.Id where f.accepted=0 and f.user_id="+id+")";
	...
	public boolean addFriend(int userId,int friendId)
	{
		...
		{
		 java.sql.Statement stmt=con.createStatement();
		 String query="insert into friends(user_id,friend_id,createdOn,accepted) values("+userId+","+friendId+","+LocalDate.now()+",true)";
